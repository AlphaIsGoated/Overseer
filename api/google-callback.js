...
// and bounces back to /calendar.html with the tokens in the URL
// hash. The hash never reaches the server — only the browser
// reads it, then stores the tokens in localStorage. Mirrors the
// WHOOP/Strava callbacks elsewhere in this dashboard.
// Env vars required on Vercel:
//   GOOGLE_CLIENT_ID
//   GOOGLE_CLIENT_SECRET
// (no GOOGLE_REDIRECT_URI env var needed — derived from the live
// host below, same reasoning as the WHOOP/Strava callbacks: it must
// exactly match whatever redirect_uri was used at the authorize step.)
// ============================================================
export default async function handler(req, res) {
  ...
  try {
    ...
    res.writeHead(302, { Location: '/calendar.html#' + hash });
    res.end();
  } catch (e) {
