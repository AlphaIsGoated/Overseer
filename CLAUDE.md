...
|------|--------|--------|-----------------|
| 2026-07-27 | *(this session)* | Three fixes: (1) Added `replace_plan` op to marathon `executeCoachAction()` — replaces ALL `plan.entries` atomically with a new set. Coach must use this instead of multiple `add_entry` calls when generating a new plan, so old entries don't persist. `CHAT_SYS()` updated with schema. (2) Fixed proactive status sweep running on every page load when localStorage is full. Root cause: `localStorage.setItem('coach_prompt_build', ...)` failed silently on quota → version check fired on every load → proactive key deleted every load → scan triggered every open. Fixes: `pruneOldStorage()` called before the COACH_PROMPT_BUILD check; retry-on-quota for `coach_prompt_build` save; proactive key is now set BEFORE `addMsg()` in `runProactiveScan()` (was after, so a chat-history quota failure left the key unset). `COACH_PROMPT_BUILD` bumped to `'2026-07-27-v4'`. (3) Fixed `[COACH_ACTION:...]` JSON parsing truncating payloads that contain `]` characters (e.g. `replace_plan`'s entries array). Simple lazy regex `/\[COACH_ACTION:([\s\S]*?)\]/` stops at the first `]` inside the JSON. Replaced with a bracket-depth parser (`extractCoachActions` / `stripCoachActions`) that finds the true closing `]`. | `replace_plan` discards ALL existing entries including completed runs — coach should only use it for full replanning, never for minor adjustments. After a `replace_plan`, completed history is lost from the plan (Strava remains the source of truth for actual run history). |
